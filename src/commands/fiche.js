...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'view') {
      ...
      const embed = buildFicheEmbed(fiche, targetUser);
      if (allFiches.length > 1) embed.setFooter({ text: `Fiche ${index + 1} / ${allFiches.length} — ${targetUser.username}` });
      const ficheButtons = buildFicheButtons(targetUser.id);
      const components = [...ficheButtons];
      if (allFiches.length > 1) components.push(buildNavigationButtons(index, allFiches.length, targetUser.id, targetUser.id));
