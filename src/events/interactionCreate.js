...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      // Non-admin : uniquement /fiche (sans sous-commande, = voir sa propre fiche)
      if (!isAdmin) {
        if (cmdName !== 'fiche') {
          return interaction.reply({ content: '❌ Seuls les administrateurs peuvent utiliser cette commande.', ephemeral: true });
        }
        // Non-admin et /fiche → afficher sa propre fiche
        try {
          await interaction.deferReply({ ephemeral: true });
          const userId = interaction.user.id;
          // Récupère toutes les fiches du joueur (tous mondes)
          ...
          if (fiches.length > 1) components.push(buildNavigationButtons(index, fiches.length, userId));
    ...
    if (interaction.isButton()) {
      ...
      if (id.startsWith('nav_prev_') || id.startsWith('nav_next_')) {
        ...
        // Cherche la session dont le message correspond à ce message
        ...
        const fiche = monde
          ? await getFicheByMonde(userId, monde)
          : await getFiche(userId);
        let targetUser;
        try { targetUser = await interaction.client.users.fetch(userId); }
        catch { targetUser = { username: 'Joueur inconnu', displayAvatarURL: () => null }; }
        const embed = buildFicheEmbed(fiche, targetUser);
        const label = session ? `Session #${session.sessionNum} (Monde ${session.monde})` : 'Fiche';
