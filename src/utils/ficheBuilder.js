...
function fmtHp(hp, maxHp) {
  const max = maxHp ?? 5;
  const val = Math.max(0, Math.min(max, hp ?? max));
  if (val === 0) return `💀 0/${max}`;
  return '❤️'.repeat(Math.min(val, 20)) + ` (${val}/${max})`;
}

// ─── Build embed ───────────────────────────────────────────────────────────────
function buildFicheEmbed(fiche, targetUser) {
  const stats = fiche.competences || { intelligence: 0, force: 0, dexterite: 0, chance: 0 };
  const monde = fiche.monde ?? 1;

  const embed = new EmbedBuilder()
    .setTitle(`Fiche personnage de ${targetUser.username} — 🌍 Monde ${monde}`)
    .setColor(0x2b2d31)
    .addFields(
      ...
      { name: '\u200B',          value: '\u200B',                           inline: false },
      { name: '❤️ Vie (HP)',     value: fmtHp(fiche.hp, fiche.maxHp),      inline: true },
...
// Boutons lecture seule (non-admin) : uniquement Refresh + navigation éventuelle
function buildFicheButtonsReadonly(userId) {
...
// targetUserId : si fourni, mode "view user" — navigation entre les fiches d'un seul joueur (multi-monde)
function buildNavigationButtons(currentIndex, total, currentUserId, targetUserId = null) {
  const onlyOne = total <= 1;
  const suffix = targetUserId ? `__view_${targetUserId}` : '';
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`nav_prev_${currentIndex}_${currentUserId}${suffix}`)
      .setLabel('◀ Précédente')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(onlyOne),
    new ButtonBuilder()
      .setCustomId(`nav_next_${currentIndex}_${currentUserId}${suffix}`)
      .setLabel('Suivante ▶')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(onlyOne),
  );
...
function createDefaultFiche(nom, age, taille, descriptif, competences, monde = 1) {
