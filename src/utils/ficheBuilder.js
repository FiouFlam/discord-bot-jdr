...
function buildFicheEmbed(fiche, targetUser) {
  ...
  const inventaire = fiche.inventaire.length > 0
    ? fiche.inventaire.map((obj, i) => `${i + 1}. ${obj.nom} *(niv. ${obj.niveau})*`).join('\n')
    : '*(vide)*';
  embed.addFields({ name: '🎒 Inventaire', value: inventaire, inline: false });

  // Inventaire propriétés
  const proprietes = fiche.proprietes.length > 0
    ? fiche.proprietes.map((p, i) => `${i + 1}. ${p}`).join('\n')
    : '*(vide)*';
  embed.addFields({ name: '🏡 Liste des propriétés', value: proprietes, inline: false });
...
function buildArgentButtons(userId) {
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
...
module.exports = { buildFicheEmbed, buildFicheButtons, buildNavigationButtons, buildArgentButtons, createDefaultFiche };
