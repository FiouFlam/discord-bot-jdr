...
function deleteSession(guildId, sessionNum) {
  const result = sessions.delete(`${guildId}_${sessionNum}`);
  // Reset le compteur si plus aucune session active pour cette guilde
  const remaining = getSessionsByGuild(guildId);
  if (remaining.length === 0) {
    sessionCounters.delete(guildId);
  }
  return result;
}
